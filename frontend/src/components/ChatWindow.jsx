...
function ChatWindow({ currentUser, otherUser, patientId }) {
  ...
  return (
    <div className="flex flex-col h-[500px] sm:h-[600px] max-h-[75vh] bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
      {/* Chat Header */}
      <div className="p-3 sm:p-4 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="relative">
            <img src={otherUser.avatar} alt={otherUser.name} className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-white shadow-sm" />
            <div className="absolute bottom-0 right-0 w-2.5 h-2.5 sm:w-3 sm:h-3 bg-green-500 border-2 border-white rounded-full"></div>
          </div>
          <div>
            <h4 className="font-bold text-slate-800 text-sm sm:text-base leading-tight">{otherUser.name}</h4>
            <p className="text-[11px] sm:text-xs text-slate-500 capitalize">{otherUser.role}</p>
          </div>
        </div>
      </div>
          ...
          chatMessages.map((msg) => {
            ...
            return (
              <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'} animate-fade-in`}>
                <div className={`max-w-[88%] sm:max-w-[80%] ${isMe ? 'order-2' : 'order-1'}`}>
                  <div className={`px-3.5 sm:px-4 py-2.5 sm:py-3 rounded-2xl shadow-sm text-xs sm:text-sm ${
                    isMe 
