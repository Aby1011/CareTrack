...
export const addVitalRecord = (patientId, recordData) => {
  const record = {
    ...recordData,
    patientId,
    id: vitalRecords.length > 0 ? Math.max(...vitalRecords.map(v => v.id || 0)) + 1 : 1,
    date: recordData.date || new Date().toISOString().split('T')[0],
    timestamp: new Date().toISOString()
  };
  vitalRecords.push(record);
...
export const canSubmitCheckIn = (patientId) => {
  const patient = users.patients.find(p => p.id === patientId);
  if (!patient || !patient.followUpPlan) return false;

  const today = new Date().toISOString().split('T')[0];
  const patientVitals = vitalRecords.filter(v => v.patientId === patientId);
  const alreadySubmitted = patientVitals.some(v => v.date === today);
...
export const followUpPlanAPI = {
  create: (patientId, planData) => {
    const patient = users.patients.find(p => p.id === patientId);
    if (patient) {
      patient.followUpPlan = { ...planData, createdAt: new Date().toISOString() };
      saveUsers();
      return true;
    }
    return false;
  ...
  update: (patientId, planData) => {
    const patient = users.patients.find(p => p.id === patientId);
    if (patient) {
      patient.followUpPlan = { ...patient.followUpPlan, ...planData, updatedAt: new Date().toISOString() };
      saveUsers();
      return true;
    }
    return false;
...
export const prescriptionAPI = {
  create: (patientId, doctorId, prescriptionData) => {
    const patient = users.patients.find(p => p.id === patientId);
    if (patient) {
      const newPrescription = {
        ...
        signature: 'CRYPTO_SIG_' + Math.random().toString(16).substr(2, 24)
      };
...
export const loadUsers = () => {
  try {
    // FORCE RESET: For this session, we clear storage to remove unwanted data
    // localStorage.removeItem('caretrack_users'); 
    ...
    return {
      ...
      doctors: [...initialUsers.doctors, ...(parsed.doctors || []).filter(d => d.email.endsWith('@system.com') && !initialUsers.doctors.find(id => id.id === d.id))],
      nurses: [...initialUsers.nurses, ...(parsed.nurses || []).filter(n => n.email.endsWith('@system.com') && !initialUsers.nurses.find(inr => inr.id === n.id))],
      patients: [...initialUsers.patients, ...(parsed.patients || []).filter(p => p.email.endsWith('@email.com') && !initialUsers.patients.find(ip => ip.id === p.id))]
    };
  } catch (e) {
    return initialUsers;
  }
...
export const checkVitalsForAlerts = (patientId, vitals) => {
  ...
  if (systolic > alertThresholds.systolic.max || systolic < alertThresholds.systolic.min) {
    issues.push(`Blood Pressure (${systolic}/${diastolic})`);
    severity = 'critical';
  }
  if (bloodSugar > alertThresholds.bloodSugar.max || bloodSugar < alertThresholds.bloodSugar.min) {
    issues.push(`Blood Sugar (${bloodSugar})`);
  ...
  if (issues.length > 0) {
    addAlert({
      patientId,
      type: severity,
      message: `${severity === 'critical' ? 'Critical' : 'Warning'}: Abnormal ${issues.join(', ')} detected.`,
    });
  }
};
