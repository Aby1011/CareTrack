...
function PatientDashboard({ user, onLogout }) {
  ...
  const Sidebar = () => (
    <>
      ...
      <div className={`w-64 bg-gradient-to-b from-teal-600 to-teal-800 text-white h-screen flex flex-col fixed left-0 top-0 z-50 transition-transform duration-300 ${
        sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
      }`}>
        <div className="p-6 border-b border-teal-500/30 flex items-center justify-between">
          ...
          <button
            onClick={() => setSidebarOpen(false)}
            className="lg:hidden p-1.5 text-teal-200 hover:text-white rounded-lg hover:bg-white/10"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-4 border-t border-teal-500/30">
          ...
          >
            ...
            <div className="min-w-0">
              <p className="font-medium text-sm group-hover:text-white transition-colors truncate">{user.name}</p>
              <p className="text-xs text-teal-200">Patient</p>
            </div>
          </div>
          <button
            onClick={onLogout}
            className="w-full flex items-center justify-center gap-2 py-2 px-4 bg-white/10 hover:bg-white/20 rounded-xl transition-colors text-teal-100 hover:text-white backdrop-blur text-sm"
          >
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="lg:ml-64">
        <header className="bg-white border-b border-slate-200 px-4 sm:px-6 lg:px-8 py-3 sm:py-4 sticky top-0 z-10 animate-header">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <div className="relative cursor-pointer hover:bg-slate-100 p-2 rounded-full transition-colors group">
                ...
                {showNotifications && (
                  <div className="absolute right-0 mt-3 w-72 sm:w-80 bg-white rounded-2xl shadow-2xl border border-slate-100 z-50 animate-scale-up overflow-hidden origin-top-right">
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 min-h-screen max-w-7xl mx-auto">
