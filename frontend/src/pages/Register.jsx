...
const Register = () => {
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex items-start md:items-center justify-center p-4 sm:p-6 md:p-8 pt-16 sm:pt-20 md:py-12 relative">
            <Link
                to="/"
                className="absolute top-4 left-4 sm:top-6 sm:left-6 flex items-center gap-2 px-3 sm:px-4 py-2 bg-white hover:bg-slate-50 border border-slate-200 text-slate-600 rounded-xl transition-all duration-200 group z-50 shadow-sm hover:shadow-md text-xs sm:text-sm"
            >
                <Home className="w-4 h-4" />
                <span className="font-medium">Home</span>
            </Link>
            <div className="max-w-2xl w-full bg-white rounded-[2rem] sm:rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col md:flex-row border border-slate-100">
                {/* Left Side - Info */}
                <div className="md:w-5/12 bg-slate-900 p-6 sm:p-10 text-white flex flex-col justify-between relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        <Link to="/" className="flex items-center gap-2 mb-6 sm:mb-12 hover:opacity-80 transition-opacity">
                            <div className="w-8 h-8 bg-teal-500 rounded-lg flex items-center justify-center shadow-lg shadow-teal-500/20">
                                <Heart className="w-5 h-5 text-white" />
                            </div>
                            <span className="text-lg sm:text-xl font-bold tracking-tight">{systemName}</span>
                        </Link>

                        <h2 className="text-2xl sm:text-4xl font-bold mb-3 sm:mb-6 leading-tight">Join our <br className="hidden sm:inline" />care network.</h2>
                        ...
                        <div className="space-y-4 sm:space-y-6">
                            <div className="flex items-center gap-4 group">
                                ...
                                <p className="text-xs sm:text-sm font-medium text-slate-300">Secure & Confidential</p>
                            </div>
                            <div className="flex items-center gap-4 group">
                                <div className="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center group-hover:bg-teal-500/20 group-hover:border-teal-500/30 transition-all">
                                    <Activity className="w-5 h-5 text-teal-400" />
                                </div>
                                <p className="text-xs sm:text-sm font-medium text-slate-300">Real-time Monitoring</p>
                            </div>
                        </div>
                    </div>

                    <div className="relative z-10 mt-6 sm:mt-12 pt-6 sm:pt-12 border-t border-white/10 text-slate-500 text-[10px] sm:text-xs">
                        © 2026 {systemName} Health Systems.
                    </div>
                </div>

                {/* Right Side - Form */}
                <div className="md:w-7/12 p-6 sm:p-8 md:p-12 relative">
                    ...
                    <form onSubmit={(role === 'nurse' || role === 'doctor') && step === 1 ? handleNextStep : handleSubmit} className="space-y-4 sm:space-y-6">
                            ...
                            <div className="grid grid-cols-3 gap-2 sm:gap-3">
                                {['patient', 'doctor', 'nurse'].map(r => (
                                    <button
                                        ...
                                        className={`py-2.5 sm:py-3 rounded-2xl text-xs sm:text-sm font-bold capitalize transition-all border ${role === r
